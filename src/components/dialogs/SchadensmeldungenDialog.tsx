...
interface SchadensmeldungenDialogProps {
  ...
  defaultValues?: Schadensmeldungen['fields'];
  straßenverzeichnisList: Strassenverzeichnis[];
  enablePhotoScan?: boolean;
}

export function SchadensmeldungenDialog({ open, onClose, onSubmit, defaultValues, straßenverzeichnisList, enablePhotoScan = false }: SchadensmeldungenDialogProps) {
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      const schema = `{\n  "strasse": string | null, // Name des Straßenverzeichnis-Eintrags (z.B. "Jonas Schmidt")\n  "schadenstyp": string | null, // one of: "Schlagloch", "Riss/Spalte", "Absenkung", "Aufbruch", "Verschleiß der Fahrbahndecke", "Beschädigte Bordsteinkante", "Sonstiges" // Schadenstyp\n  "schweregrad": string | null, // one of: "Gering", "Mittel", "Hoch", "Kritisch" // Schweregrad\n  "hausnummer_bereich": string | null, // Hausnummer/Bereich\n  "beschreibung": string | null, // Beschreibung des Schadens\n  "meldedatum": string | null, // YYYY-MM-DD // Meldedatum\n  "status": string | null, // one of: "Gemeldet", "In Bearbeitung", "Behoben", "Abgelehnt" // Status\n  "melder_vorname": string | null, // Vorname\n  "melder_nachname": string | null, // Nachname\n  "melder_email": string | null, // E-Mail\n  "melder_telefon": string | null, // Telefon\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["strasse"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = _lookupOptions[k] ? _resolveLookup(k, v) : v;
        }
        const strasseName = raw['strasse'] as string | null;
        if (strasseName) {
          const strasseMatch = straßenverzeichnisList.find(r => matchName(strasseName!, [String(r.fields.strassenname ?? '')]));
          if (strasseMatch) merged['strasse'] = createRecordUrl(APP_IDS.STRASSENVERZEICHNIS, strasseMatch.record_id);
        }
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="w-full max-w-lg max-h-[90vh] overflow-y-auto overflow-x-hidden">
        <DialogHeader>
          <DialogTitle>{defaultValues ? 'Schadensmeldungen bearbeiten' : 'Schadensmeldungen hinzufügen'}</DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Select
              value={extractRecordId(fields.strasse) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, strasse: v === 'none' ? undefined : createRecordUrl(APP_IDS.STRASSENVERZEICHNIS, v) }))}
            >
              <SelectTrigger id="strasse"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-2">
            <Label htmlFor="schadenstyp">Schadenstyp</Label>
            <Select
              value={lookupKey(fields.schadenstyp) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, schadenstyp: v === 'none' ? undefined : v as any }))}
            >
              <SelectTrigger id="schadenstyp"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
                ...
                <SelectItem value="verschleiss">Verschleiß der Fahrbahndecke</SelectItem>
                <SelectItem value="bordstein">Beschädigte Bordsteinkante</SelectItem>
          ...
          <div className="space-y-2">
            <Label htmlFor="hausnummer_bereich">Hausnummer/Bereich</Label>
            <Input
              id="hausnummer_bereich"
              value={fields.hausnummer_bereich ?? ''}
              onChange={e => setFields(f => ({ ...f, hausnummer_bereich: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="gps_position">GPS-Position</Label>
            <div className="space-y-3">
              <Button type="button" variant="outline" className="w-full" disabled={locating} onClick={() => geoLocate("gps_position")}>
                {locating ? <Loader2 className="h-4 w-4 mr-1.5 animate-spin" /> : <Crosshair className="h-4 w-4 mr-1.5" />}
                Aktuellen Standort verwenden
              </Button>
              {fields.gps_position?.info && (
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <Input type="number" step="any"
                      value={fields.gps_position?.lat ?? ''}
                      onChange={e => {
                        const v = e.target.value;
                        setFields(f => ({ ...f, gps_position: { ...(f.gps_position as any ?? {}), lat: v ? Number(v) : undefined } }));
                      }}
                    />
                  </div>
                  <div>
                    ...
                    <Input type="number" step="any"
                      value={fields.gps_position?.long ?? ''}
                      onChange={e => {
                        const v = e.target.value;
                        setFields(f => ({ ...f, gps_position: { ...(f.gps_position as any ?? {}), long: v ? Number(v) : undefined } }));
                      }}
                    />
                  </div>
                </div>
          ...
          <div className="space-y-2">
            <Label htmlFor="beschreibung">Beschreibung des Schadens</Label>
            <Textarea
              id="beschreibung"
          ...
          <div className="space-y-2">
            <Label htmlFor="melder_email">E-Mail</Label>
            <Input
              id="melder_email"
              type="email"
          ...
          <div className="space-y-2">
            <Label htmlFor="melder_telefon">Telefon</Label>
            <Input
              id="melder_telefon"
